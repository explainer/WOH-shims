...
class response {
  ...
  public function __construct( $query_value )
  {
    ...
    // add the txn_amount value
    $this->post['txn_amount'] = $this->post['mc_gross'];
    ...
    if ( $this->is_verified() ) {
      do_action( 'pdbmps-ipn_response_data', apply_filters( 'pdbmps-template_data', $this->data() ) );
    }
  }
  ...
  {
    $record = array();
    if ( isset( $this->post['custom'] ) && $participant = \Participants_Db::get_participant( \pdbmps\Plugin::get_record_id_from_return_code( $this->post['custom'] ) ) ) {
      $record = $participant;
    }
